...
describe.skipIf(!HAS_MVN)(
  'resolveMaven — dependency-backed Spring MockMvc discovery (P4R-09)',
  () => {
    let spring: string;
    ...
    beforeAll(() => {
      ...
      writeFileSync(
        ...
        [
          ...
          '    Files.writeString(Path.of("target", "p4r-09-mockmvc-executed"), "executed");',
          '  }',
          '}',
          '',
        ].join('\n'),
    ...
    it('uses Maven test dependencies, returns the exact source file, and never executes the body', () => {
      ...
      expect(existsSync(join(spring, 'target', 'p4r-09-mockmvc-executed'))).toBe(false);
    }, 300_000);
  },
);
