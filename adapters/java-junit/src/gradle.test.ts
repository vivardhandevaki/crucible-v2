...
describe.skipIf(!HAS_GRADLE)('resolveGradle — configured Test.classpath (P4R-09)', () => {
  ...
  beforeAll(() => {
    ...
    appendFileSync(
      ...
      [
        '',
        'configurations { p4r09DiscoveryOnly }',
        'dependencies {',
        "  testCompileOnly 'org.apache.commons:commons-lang3:3.17.0'",
        "  p4r09DiscoveryOnly 'org.apache.commons:commons-lang3:3.17.0'",
        '}',
        "tasks.named('test') { classpath = classpath + configurations.p4r09DiscoveryOnly }",
        '',
      ].join('\n'),
      'utf8',
    );
    ...
    writeFileSync(
      ...
      [
        ...
        '    Files.writeString(Path.of("build", "p4r-09-custom-classpath-executed"), "executed");',
        '  }',
        '}',
        '',
      ].join('\n'),
  ...
  it('uses the evaluated task classpath, returns the exact source file, and never executes the body', () => {
    ...
    expect(existsSync(join(customized, 'build', 'p4r-09-custom-classpath-executed'))).toBe(false);
  }, 300_000);
});
