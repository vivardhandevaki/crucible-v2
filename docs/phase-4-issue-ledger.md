...
| P4-008 | On the Notes validation host, even a direct Codex `workspace-write` session cannot start filesystem commands because Bubblewrap cannot create its loopback interface. The P4-09 full-access opt-in is intentionally unacceptable to an operator who does not want to remove that boundary. | Design resolved; implementation queued | P4-10 ratifies session-native propose/revise + implement as a separate local mode outside `AgentSubstrate`, with CLI-owned handoffs and unchanged approval/verify/CI authority. P4-08 is absorbed into the same implementation cluster; architecture §10 and the Phase 4 runbook hold the lifecycle and recovery contract. |
