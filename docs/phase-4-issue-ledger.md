...
| P4-007 | Nested Codex propose sessions cannot use `workspace-write` on the Notes validation host: bubblewrap fails to create its loopback interface (`bwrap: loopback: Failed RTM_NEWADDR: Operation not permitted`), so the role cannot read or write and produces an empty bundle. | Pending design confirmation | P4-09 proposes a narrow unblocker: retain `workspace-write` by default and allow `danger-full-access` only through an explicit gitignored local opt-in. No automatic fallback, CI change, or enforcement change is allowed. P4-08 skills are not a standalone fix because a thin wrapper would still spawn the same nested session. |
