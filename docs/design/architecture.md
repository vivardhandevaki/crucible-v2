...
## 22. Manual solo-posture root bootstrap (P4-26, ratified 2026-08-16)

The Notes P4-22 target has no explicit review policy, so strict parsing defaults both PR reviewer controls to required. Its missing API credential is therefore a correct red detached-review result, not evidence that advisory mode should be inferred. The exact P4-25 bridge fixes workflow authority but cannot itself make the target-owned required reviewer green. A normal P4-18 config PR is also impossible: target-owned policy deliberately reads the old required target before it reads the candidate advisory config.

P4-26 is a narrowly named manual root transaction, not a new ordinary authority classification. The future review-posture bootstrap command runs only in a clean checkout whose committed target matches the exact P4-25 legacy bridge: known old pin, exact advisory local-review setting, required-mode main and detached-review template bytes, tracked-clean tree, and no active approval sealing the old lock. It computes all expected bytes before writing, validates the transaction allowlist, then atomically stages exactly five paths: framework lock, main workflow, deletion of detached review workflow, crucible.yaml, and settings.yaml. The source must be immutable and reachable; the final bytes are rendered from that source for CI/human advisory plus required local review. There is no flag for another mode or path.

No candidate check is claimed as authority. The operator is shown the exact source pin, preconditions, five paths, and manual procedure: remove verify temporarily; verify the generated diff and green source framework release; perform fresh local Codex review and deterministic verification; merge; restore verify; then validate a fresh target checkout. The local verdict remains convenience evidence, not CI evidence. The mandatory human comparison is the root approval for this exceptional TCB transition. The final target owns ordinary future enforcement: advisory mode omits the detached reviewer and route, while verify remains required. The command, templates, classifier, and tests must reject a changed product/test/bundle/adapter/schema/skill path, an explicit or malformed legacy review policy, a non-exact bridge, wrong source/pin, workflow drift, partial write, stale local verdict, and every tool, Git, or parser failure.
