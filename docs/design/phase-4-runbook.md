...
## P4-26 no-API solo-posture root bootstrap

The preserved Notes target after P4-25 phase one still defaults to required CI and human review. A red detached judge without an API key is correct. Do not merge the P4-25 finalization PR around that red check, edit a candidate config to make it pass, upload an empty reviewer batch, or call the failure advisory.

After P4-26 is released, abandon the P4-25 finalization PR as superseded and use a fresh clean worktree at the exact merged P4-25 bridge. Run only the review-posture bootstrap command with an explicit root acknowledgement. It must report exactly the immutable new pin, final main workflow, review-workflow removal, strict advisory/advisory enforcement block, and required-local-review settings block. Any other output is a stop condition.

Before the one manual root merge, temporarily remove verify from GitHub required checks; compare the five generated paths and source release; run deterministic local verification and a fresh local Codex review; merge; and restore verify immediately. Then inspect a fresh target checkout with doctor. The next ordinary PR must show target-owned advisory posture: verify runs, while detached review and route are absent rather than skipped. Only then open the fresh create-note flow. This exception is unavailable after the exact legacy target is gone.
