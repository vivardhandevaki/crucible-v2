...
For each phase, inspect the command's machine-readable classification and `git diff --name-status`. Phase one may contain only the new lock and exact transition workflow bytes. Phase two may contain only exact final managed-workflow bytes and must be judged by the target-owned transition workflow. Any config, settings, skill, schema, adapter, product, test, OpenSpec bundle, untracked-to-tracked, or unrelated path stops the rollout. A missing expected `verify` context also stops; it is not an invitation to merge around branch protection.
