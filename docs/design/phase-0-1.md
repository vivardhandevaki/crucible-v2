...
- **propose:** args = change name + intent text. Scaffolds the OpenSpec change (spike-determined: `openspec new change <name> --schema crucible --json`; change names are lowercase kebab-case, no leading digit — propose validates the same rule), invokes substrate role=propose with `.crucible/context/propose.md` (P1 version of the prompt: emit valid bundle incl. oracles per grammar + bound test files in the toy repo's convention; include Unspecified/Seams sections). Then validates the bundle (parsers §3) and runs lint (§8) + adapter resolve; red → exit 1 with report (agent output is judged, not trusted). *(Settled P1-09:)* the judgment reuses the `VerifyReport` shape — a `bundle` check (parse failures fold into red findings, since post-session artifacts are the agent's product under judgment, not fail-closed input) then `traceability`, lint gated on a green bundle. Pre-session defects stay fail-closed: bad name / empty intent → exit 3, existing change dir → exit 2 (`--revise` is P2-05), missing role prompt → exit 2, scaffolder that produced no dir → exit 3. Model routed via convenience `models.propose` (default in the CLI shim); transcript minted by the command per architecture §6. The P1 role prompt lives in the toy repo (`fixtures/toy-repo/.crucible/context/propose.md`) — per-project TCB file; shipping/installation is `init`'s job (P2).
...
- **implement:** preconditions: approval.yaml present + valid (§4) else exit 2 — a *void* seal (a sealed file edited/missing since approval) also refuses at exit 2, listing the mismatches (not a red verify verdict: implement must not run at all against a broken seal). Generates `tasks.md` via substrate (role=implement, first action) then runs the implement session; afterwards runs local verify and reports. Does not loop in P1 (single session — the implement↔verify *iteration budget* is P2; P1 still runs two role=implement sessions: one to author `tasks.md`, one to implement). *(Settled P1-13:)* the two sessions are structurally separated so "tasks.md first" is a command guarantee, not a trusted agent claim — between them the command asserts `tasks.md` exists on disk (invariant 2); a tasks session that produced none → fail-closed exit 3 (a required downstream artifact is absent, invariant 3), not a report finding. The local verify reuses the P1-12 core; a red verdict is exit 1 at the CLI (a session that edits a sealed test file is caught by verify's approval-hash check). The P1 role prompt lives in the toy repo (`fixtures/toy-repo/.crucible/context/implement.md`) — per-project TCB file; shipping/installation is `init`'s job (P2).
...
- **status:** derives phase from artifact presence/validity (+ approval hash check), prints next command, reconciles state.yaml. Warns on config-differs-from-merge-base. *(Settled P1-14:)* the derived phase ladder is coarse and adapter-free — `absent → proposed → approval-void → approved → implemented` — from presence + the parse grammars + the approval hash recompute; status runs NO adapter (so `proposed` means "artifacts parse", not "lint green" — verify/approve own the resolver-backed lint) and always exits 0 (invariant 11: it blocks nothing). "implemented" is deliberately coarse: without running oracles status cannot tell a passing implementation from a red one (verify grades that). Reconciliation is conservative: state.yaml is rewritten only when missing, corrupt, or its recorded phase is *impossible* given the artifacts — a "rung" comparison so the finer red/green labels the commands record (`implement-red`, `propose-red`) are never clobbered by the coarse derived phase; a consistent file (and its event log) is left byte-for-byte. The corrupt-state tolerance lives in `state/reconcile.ts`, isolated from the strict `state.ts` parser so no enforcement path can accept a broken log. Fail-closed still holds for the *sealed* artifacts: a malformed bundle artifact or corrupt approval.yaml propagates exit 3 from its parser (invariant 3) — only the derived state.yaml is repaired, never a TCB artifact. The merge-base config read (git) is the one injected edge (`StatusDeps.readMergeBaseConfig`); best-effort in the CLI shim — any git failure yields no warning rather than a blocker.
- **CI template (`ci-templates/crucible.yml`):** checkout PR; checkout target-branch crucible.yaml separately; `crucible verify --config-from ...` as a required check. Installed by hand in P1 (`init` polish is P2). *(Settled P1-15:)* the shipped workflow triggers on `pull_request`, extracts `crucible.yaml` from `origin/<base_ref>` into `$RUNNER_TEMP` (outside the PR tree, so the PR's own copy can never shadow it), detects touched `openspec/changes/<name>/` bundles from the diff, and runs `npx crucible --config-from <target-dir> verify <change>` with **no** `continue-on-error`/`|| true` (fail-closed: a red verdict → red check). **Honest P1 boundary:** verify does not yet *consume* enforcement config (tier/risk computation is P2-02/P2-03), so `--config-from` is wired but inert, and the live adapter pin (`crucible init`) is P2 — a real consumer run reaches the adapter-pin precondition until then. P1 delivered + tested the *structural* half of the acceptance (target-branch sourcing + fail-closed gate, asserted by `ci-templates/src/crucible-template.test.ts` parsing the workflow); the **executable** proof — a PR that loosens a risk glob and still routes to human because CI used the target-branch config — required verify to read config and was tracked as a distinct task, **P2-18** (depends on P2-03). *(Folded back in P2-18:)* verify now consumes `--config-from` (P2-03) and the executable half is delivered — `core/test/target-branch-rule.test.ts` is invariant #7's permanent behavioral anchor (the analog of P1-16 for the tracer): a hermetic test-repo where the loosened-glob PR still routes human under the extracted target-branch config, auto under its own config, and auto once the loosening has merged. **The "structural-only" boundary above is now historical** — the behavioral proof exists. This split was deliberate and recorded rather than amended into P1-15's acceptance (see `docs/tasks/phase-2.md` P2-18).
