...
**P4-24 · Target-owned executable CI contract** · Tier: Fable / Sol (CI authority and release-qualification design) + Opus / Terra (implementation) · Trigger: the P4-23 Notes pin PR #19 failed before framework checkout because the snapshot producer wrote `.crucible/framework.lock.json` while the next generated step read the removed root-level path. P4-23's ratified real-Git consumer, worked verification, and bootstrap acceptance tests were not implemented, and the `pull_request` trigger also allowed a PR to execute its proposed workflow despite P4-16's target-owned claim.

Reads: charter §The Target-Branch Rule, §Configuration & Reviewer Law, P4-14/P4-16/P4-18/P4-23/P4-24 amendments; architecture.md §§11, 13, 15, 19, and 20; design/phase-0-1.md §Target-branch rule; design/phase-4-runbook.md §Review posture and local reviewer rollout, §Target enforcement snapshot recovery, and §Target-owned CI recovery; P4-16/P4-23 above; AGENTS.md invariants 1–3, 5–9, 11–12; issue ledger P4-022.

Delivers: a base-owned, credential-free mechanical workflow; one atomic exact-SHA snapshot-and-pin handoff; a separately recomputed deterministic route/ratchet edge that never trusts a candidate-executing job; and executable generated-workflow qualification. It does not alter product behavior, artifact/seal/oracle law, tier rules, review posture, detached reviewer semantics, branch settings, or permit a red check to be merged as green.

**Decision ratified 2026-08-15:** replace the candidate-owned `pull_request` harness with a base-owned `pull_request_target` mechanical harness that executes candidate code only in a no-secret/no-write job. Mint one fresh snapshot root and exact base/head binding, expose it solely through step outputs, and remove every reconstructed consumer path. Recompute routing and override payloads in a separate target-pinned, non-executing job. Make exact generated scripts—not paraphrased replicas—the subject of real-Git and worked-consumer tests.

Acceptance (write every test red before production changes):

- generic and Java template tests prove the workflow is loaded from the base event, binds exact event base/head SHAs, checks out candidate code with persisted credentials disabled, and gives the candidate-executing verify job no secret, issue-write, or pull-request-write authority;
- the bootstrap step alone creates a fresh snapshot, validates and extracts the fixed P4-23 layout, strictly parses `<snapshot>/.crucible/framework.lock.json`, and emits the only `snapshot/repository/commit/base_sha/head_sha` outputs; every checkout/config/verify consumer uses those outputs and no legacy/reconstructed/mutable-branch path exists;
- an executable real-Git harness runs the exact parsed bootstrap and pin-consumer scripts for both templates and all four init postures, proves optional review-workflow presence/absence and bytes come only from the base SHA, and proves hostile PR config/lock/workflow replacements are inert;
- failure cases cover absent config/lock/main workflow, unexpected or missing review workflow for policy, executable/symlink/submodule/tree entries, malformed/extra-key/wrong-version pin JSON, invalid repository/SHA, unfetchable or mismatched SHAs, Git/Node/tool failures, stale pre-existing temp content, missing outputs, and consumer path drift; all abort before framework checkout or verification with no fallback/partial success;
- a deterministic route command recomputes per-change tier/routing from target config plus exact base/head candidate data, aggregates multi-change routing, handles the exact no-change P4-16 lane, validates override payloads, and rejects malformed bundles/approvals/diffs; the route job ignores all outputs/files/artifacts from the candidate-executing job except its GitHub conclusion;
- permissions tests prove only the non-executing route/ratchet job can read reviews or write an override issue, mechanical verify cannot post observations, and the detached reviewer remains the sole agent-review transport with its existing secret isolation and four posture matrices;
- a worked initialized generic consumer and Spring/Java consumer execute the generated preflight, build the emitted exact framework pin, pass posture congruence, and reach ordinary traceability/oracle verification; a P4-16 pin/workflow-only PR is accepted only by the strict allowlist and a mixed config/product/change-bundle bootstrap is red;
- an acceptance-manifest test maps every P4-24 acceptance bullet to named executable tests, preventing documentation from being marked implemented while required coverage is absent; focused tests plus format, typecheck, layering lint, lint, build, full suite, deterministic packages, and a local Notes-shaped bootstrap simulation pass before opening the replacement Notes pin PR.

Rollout: preserve Notes PRs #8, #18, and #19 as evidence and merge none of them. After P4-24 design ratification and implementation, close #19 as superseded, open one new P4-16 pin/workflow-only PR from Notes `main`, and merge it only when the new target-snapshot consumer check is green. Then close #18, restart/reapprove the solo-posture change from the P4-24 pin, merge it, and only then restart the `create-note` change rather than reusing PR #8's old seal.

**P4-25 · Release-qualified consumer authority** · Tier: Fable / Sol (authority/classifier/transition design) + Opus / Terra (implementation) · Trigger: P4-24 merged with a green 961-test framework suite, but the first clean Notes upgrade simulation proved that full `init` emits forbidden config and skill bytes. Audit then found that no-bundle product/config changes pass the workflow, CI accepts missing approvals, override ratchet filing and advisory-human rejection are absent, fork/legacy-trigger migration is untested, archive-only lifecycle is unclassified, and most ratified P4-24 acceptance bullets have no executable test.

Reads: charter §The Workflow, §Approval/Amend/Override, §The Target-Branch Rule, P4-16/P4-18/P4-22/P4-24 and proposed P4-25 amendments; architecture.md §§13, 15, 18, 20–21; design/phase-2.md §§1–3 and 7; design/phase-4-runbook.md §Framework source-pin upgrades, §Review posture, §Target-owned CI recovery, and §P4-25 release recovery; AGENTS.md invariants 1–12; issue ledger P4-023.

Delivers: a dedicated transactional framework-upgrade command; a target-owned non-executing authority job and hashed handoff; deterministic governed/bootstrap/finalization/archive classification; CI-only approval-required verify/route entry points; restored override posture/ratchet behavior; fork-safe exact-SHA transport; and an executable release manifest with worked Notes qualification. It does not change artifact meaning, approval hash scope, oracle/skip law, tier computation, detached reviewer policy, local-review authority, product behavior, or infer review posture from credentials.

**Decision ratified 2026-08-15; Terra implementation authorized.** Supersede P4-24's shell bundle discovery and shared permissive command path with one typed authority boundary. Preserve local pre-approval verification, but make CI commands impossible to invoke without a validated authority manifest and approvals. Replace candidate full-init rollout with `crucible framework upgrade`; explicitly bridge the legacy `pull_request`→`pull_request_target` edge before claiming base ownership.

**Amendment ratified 2026-08-16 (legacy bootstrap):** a `pull_request`-only target cannot run target-pinned P4-25 authority for the PR that installs it. Phase one is therefore an explicit manual `legacy-bootstrap`, acknowledged through the dedicated upgrade command and limited to the new pin plus the exact bridge. Its candidate-owned job is read-only, secretless, and non-authoritative; it does not run CI authority, verification, or route. The operator temporarily removes `verify` from required checks and manually compares the two-file diff. Once merged, phase two uses the target-owned bridge, accepts only the exact bridge at the unchanged pin, and finalizes only the workflow. Lookalikes, bridge repins, and any phase-one authority claim are red.


Acceptance (write every named test red before production changes; land the manifest in the first test commit):

- upgrade-core/CLI tests cover generic and Java targets under all four review postures, exact lock/workflow-only diffs, unchanged config/settings/skills/schema/adapter/product/change bytes, active sealed-lock refusal, tracked dirt, malformed/incongruent targets, invalid/unreachable/mismatched sources, transaction rollback, deterministic rerun, ignored launcher handling, and actionable next-step output;
- classifier tests use NUL-safe real-Git diffs to accept only governed, isolated bootstrap, exact finalization, and canonical archive lanes; they reject empty/direct/mixed/lookalike/traversal/rename/binary/malformed/tool-failure cases, require a changed strict pin for bootstrap, and require valid approvals for every governed/archive input;
- authoritative verify tests prove missing/void/critical-ack-incomplete approvals are red before adapters run, seals are rechecked after execution, local pre-approval verify remains non-authoritative, archive-only PRs execute the full regression suite, and candidate code cannot supply or mutate authority evidence;
- route tests recompute from the authority manifest, cover single/multi/bootstrap/finalization/archive decisions, reject all malformed approvals/overrides/manifests/diffs, reject override under advisory-human policy, emit the exact ratchet payload under required-human policy, and prove idempotent issue filing occurs before review lookup only in the non-executing job;
- workflow tests parse and execute the exact generic/Java templates for all four postures with one authority producer and every consumer; permissions prove authority/verify have no write or secret use, route alone has review/issue authority, and no candidate-executing output crosses into route;
- exact transport tests cover same-repository and separate-fork base/head remotes, event-SHA mismatch, head absent from base origin, missing/non-regular target inputs, optional-review presence, malformed pins/manifests, stale artifacts/outputs, checkout/fetch/Git/Node/npm/adapter failures, and every downstream snapshot path;
- legacy-transition tests start from the exact Notes P4-22 trigger/pin shape, prove phase one exposes only the labelled non-authoritative legacy-bootstrap job and its mandatory manual/branch-protection procedure, prove phase two is judged by base-owned bytes, reject bridge drift, repins, and every other legacy shape, and finish at byte-identical final P4-25 workflows without branch-rule ambiguity;
- the acceptance manifest maps every P4-25 bullet and every carried-forward P4-24 bullet to discovered non-skipped test IDs; deleting/renaming/skipping a required test or adding an unlisted workflow consumer fails the meta-test;
- worked initialized generic and Spring/JUnit consumers reach traceability, current oracles, regression, seal, tier/cap, posture, route, and archive paths; hostile direct/config/product/bootstrap mixtures remain red;
- release qualification runs format, typecheck, layering lint, lint, build, full suite, deterministic packages, both workflow packages, and a built-candidate Notes mirror from current `origin/main` through authority transition, isolated pin, fresh critically approved solo posture, and a newly proposed/approved create-note flow. No Notes PR is opened until all evidence is green.

Rollout: keep Notes PRs #8 and #18 open only as failure evidence and do not update or merge their sealed branches. PR #19 is already closed. After P4-25 design ratification and implementation merge, run the dedicated command from a clean Notes worktree and follow its tested authority-transition phases. Then close #18, create a fresh solo-posture change from the final P4-25 target, and only after that merges close/restart #8 from a new approval and framework seal.

**P4-26: Manual solo-posture root bootstrap** - Tier: Fable / Sol (root-of-trust and policy-transition design) + Opus / Terra (implementation) - Trigger: P4-25 Notes phase two proved target-owned authority, verification, and route are healthy, but the preserved target defaults CI review to required and correctly rejects an absent API-backed reviewer before an ordinary candidate policy change can select advisory mode.

Reads: charter P4-15, P4-18, P4-19, P4-25, and P4-26 amendments plus Target-Branch Rule and Configuration & Reviewer Law; architecture.md sections 12, 15, 21-22; design/phase-4-runbook.md Advisory CI-review rollout, P4-25 release recovery, and P4-26 no-API solo-posture root bootstrap; AGENTS.md invariants 1-12; issue ledger P4-025.

Delivers: one explicitly acknowledged, deterministic, atomic, manual root transaction that moves the exact P4-25 Notes legacy bridge to a final solo advisory target posture without an API key. It does not weaken required review, make candidate config authoritative, add a generic config lane, change normal governed policy behavior, allow arbitrary modes or paths, turn a local verdict into CI evidence, or change product behavior.

**Decision ratified 2026-08-16; Terra implementation authorized after this design is merged.** The command accepts only the exact post-P4-25 bridge with its known pin, absent review policy, and byte-identical required templates. It stages only the new immutable lock, final advisory/advisory main workflow, deleted detached reviewer workflow, explicit advisory enforcement block, and required-local-review settings block. The one manual maintainer comparison is the root approval; every later posture transition stays a critically approved governed change.

Acceptance (write every named test red before production changes; land the manifest in the first test commit):

- command and transaction tests accept only a tracked-clean exact P4-25 bridge with known old pin, exact advisory local-review setting, exact required managed templates, reachable immutable P4-26 source, and no active seal; they produce a byte-stable five-path transaction and roll back every partial write;
- rejection tests cover explicit, unknown, malformed, or non-advisory old review fields; wrong, missing, or repinned lock; bridge/main/reviewer byte drift; missing/non-regular paths; dirty tracked or untracked-to-tracked input; source/repository/SHA failure; active approval; declined acknowledgement; changed adapter/settings/product/test/bundle/schema/skill paths; every non-advisory mode request; and parser/Git/tool failures;
- rendered generic and Java workflow tests prove the final target has target-owned verify only for advisory/advisory posture, no detached review workflow, no route job, no reviewer secret/action/batch/judge reference, and cannot expose the P4-26 root-bootstrap command in an ordinary candidate workflow;
- authority and classifier tests prove the candidate root transaction is never reported as authority, while a fresh target after manual merge treats later product/config work as ordinary governed work with target advisory policy and rejects every direct or lookalike root transaction;
- worked real-Git Notes-shaped consumers start at the exact post-#22 bridge, validate the human-visible five-file transaction, simulate the manual merge, restore verify, prove the fresh target has target-owned advisory posture, and then complete a fresh create-note proposal/approval path without touching #8, #18, #22, or #23;
- acceptance-manifest/meta, format, lint, typecheck, build, full suite, deterministic packages, workflow package, and release qualification all cover the new root transition. No Notes root-bootstrap PR is opened until every item is green.

Rollout: close Notes #23 as superseded; preserve #8 and #18 as evidence. After the P4-26 framework PR is green and merged, execute the command from a fresh clean worktree at merged #22, make the single manual five-path comparison with verify temporarily removed, merge, immediately restore verify, and validate the generated advisory target before restarting create-note. No ordinary policy PR is used for this root transition.
