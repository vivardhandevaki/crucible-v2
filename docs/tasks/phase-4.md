...
- legacy-transition tests start from the exact Notes P4-22 trigger/pin shape, prove the required `verify` context exists in transition PR one, prove transition PR two is judged by base-owned bytes, reject every other legacy shape, and finish at byte-identical final P4-25 workflows without branch-rule ambiguity;
