...
- command and transaction tests accept only a tracked-clean exact P4-25 bridge with known old pin, absent review block, exact required managed templates, reachable immutable P4-26 source, and no active seal; they produce a byte-stable five-path transaction and roll back every partial write;
- rejection tests cover explicit, unknown, or malformed old review fields; wrong, missing, or repinned lock; bridge/main/reviewer byte drift; missing/non-regular paths; dirty tracked or untracked-to-tracked input; source/repository/SHA failure; active approval; declined acknowledgement; changed adapter/settings/product/test/bundle/schema/skill paths; every non-advisory mode request; and parser/Git/tool failures;
